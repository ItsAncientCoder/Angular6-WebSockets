...
function startTimer() {
    ...
    timerId = setInterval(() => {
        ...
        var workInfo = {
            changeId: Math.floor(Math.random() * 1000000000),
            desc: " Work info description " + Math.random().toString(36).substring(10)
        };
        ...
        for (const s of sockets) {
            console.log(`Emitting value: ${workInfo}`);
            s.emit('data', { data: workInfo });
        }
